...
**ARMANDO LA CANALIZACIÓN**

Los _streams_ te permiten encadenar operaciones y expresar lo que deseas lograr en lugar de cómo hacerlo. Supongamos que queremos obtener los primeros dos nombres de nuestros amigos en orden alfabético que tengan cuatro caracteres. Sin _streams_, tendríamos que escribir algo así:
...
Esto funciona, pero se necesita cierta lectura y reflexión para entender lo que está pasando. El problema que intentamos resolver se pierde en la implementación. También está muy centrado en el *cómo* en vez del *qué*. Con _streams_, el código equivalente es el siguiente:
...
Antes de decir que es más difícil de leer, podemos formatearlo:
...
La diferencia es que expresamos lo que está sucediendo. Nos interesan los objetos de tipo `String` de longitud 4. Luego queremos ordenarlos. Después, queremos los primeros dos. Y finalmente, queremos imprimirlos. Esto se ajusta mejor al problema que estamos tratando de resolver y es más simple.

Una vez que empieces a usar _streams_ en tu código, puedes descubrir que los usas en muchos lugares. Tener código más corto, conciso y claro es definitivamente algo bueno.

En este ejemplo, se ven las tres partes de la canalización. La Figura 15.5 muestra cómo cada operación intermedia en la canalización alimenta a la siguiente.

Recuerda que el “capataz de la línea de montaje” está averiguando cómo implementar mejor la canalización del _stream_. Él prepara todas las “mesas” con instrucciones para que esperen antes de empezar. Le dice al operario de `limit()` que le informe cuando hayan pasado dos elementos. Le dice a la operaria de `sorted()` que simplemente recoja todos los elementos a medida que llegan y los ordene todos a la vez. Después de ordenar, debe empezar a pasarlos a `limit()` uno por uno. El flujo de datos se ve así:

1. El método `stream()` envía "Toby" a `filter()`. El método `filter()` ve que la longitud es la adecuada y envía "Toby" a `sorted()`. El método `sorted()` no puede ordenar todavía porque necesita todos los datos, así que retiene "Toby".
2. El método `stream()` envía "Anna" a `filter()`. El método `filter()` ve que la longitud es la adecuada y envía "Anna" a `sorted()`. El método `sorted()` no puede ordenar todavía porque necesita todos los datos, así que retiene "Anna".
3. El método `stream()` envía "Leroy" a `filter()`. El método `filter()` ve que la longitud no coincide y saca a "Leroy" del proceso de la línea de montaje.
4. El método `stream()` envía "Alex" a `filter()`. El método `filter()` ve que la longitud es la adecuada y envía "Alex" a `sorted()`. El método `sorted()` no puede ordenar todavía porque necesita todos los datos, así que retiene "Alex". En realidad, en este punto sí tiene ya todos los datos que necesita, pero todavía no lo sabe.
5. El capataz le avisa a `sorted()` que es hora de ordenar, y la ordenación ocurre.
6. El método `sorted()` envía "Alex" a `limit()`. El método `limit()` recuerda que ya ha visto un elemento y envía "Alex" a `forEach()`, que imprime "Alex".
7. El método `sorted()` envía "Anna" a `limit()`. El método `limit()` recuerda que ya ha visto dos elementos y envía "Anna" a `forEach()`, que imprime "Anna".
8. El método `limit()` ya vio todos los elementos necesarios y se lo comunica al capataz. El capataz detiene la línea y no se hace más procesamiento en la canalización.

¿Tiene sentido? Probemos unos cuantos ejemplos más para asegurarnos de que lo entiendes bien. ¿Qué crees que hace lo siguiente?
...
En realidad se queda colgado hasta que matas el programa o lanza una excepción al quedarse sin memoria. El capataz le ha indicado a `sorted()` que espere hasta que tenga todo para ordenar. Eso nunca sucede porque el _stream_ es infinito. ¿Y este otro?
...
Este imprime "Elsa" dos veces. El `filter` deja pasar los elementos, y `limit()` detiene las operaciones anteriores después de dos elementos. Ahora `sorted()` puede ordenar porque tenemos una lista finita. Finalmente, ¿qué crees que hace esto?
...
Este también se queda colgado hasta que matamos el programa. El `filter` no deja pasar nada, así que `limit()` nunca ve dos elementos. Esto significa que seguimos esperando con la esperanza de que aparezcan.

Incluso puedes encadenar dos canalizaciones. ¿Puedes identificar las dos fuentes y las dos operaciones terminales en este código?
...
Las líneas 30–32 forman una canalización, y las líneas 33 y 34 forman otra. Para la primera canalización, la línea 30 es la fuente y la línea 32 es la operación terminal. Para la segunda canalización, la línea 33 es la fuente y la línea 34 es la operación terminal. ¡Vaya forma complicada de imprimir el número 1!

> **NOTA: **En el examen, es posible que veas secuencias de preguntas largas o complejas como opciones de respuesta. Si esto sucede, concéntrate en las diferencias entre las respuestas. Esas serán tus pistas para encontrar la respuesta correcta. Este enfoque también te ahorrará tiempo al no tener que estudiar toda la secuencia de preguntas de cada opción.

Cuando veas canalizaciones encadenadas, fíjate en dónde se encuentran la fuente (_source_) y las operaciones terminales. Esto te ayudará a llevar un seguimiento de lo que está sucediendo. Incluso puedes reescribir el código en tu mente para que haya una variable intermedia y así no sea tan largo ni complicado. Nuestro ejemplo anterior se puede escribir así:
...
El estilo que utilices depende de ti. Sin embargo, necesitas ser capaz de leer ambos estilos antes de presentar el examen.


**ECHANDO UN VISTAZO DETRÁS DE ESCENA**

> El método `peek()` es útil para ver cómo funciona internamente una canalización (_pipeline_) de _streams_. Recuerda que los métodos se ejecutan contra cada elemento uno a la vez hasta que finaliza el procesamiento. Supón que tenemos este código:
...
> La fuente es un _stream_ infinito de números. Solo se permiten pasar los primeros cinco elementos antes de que el “capataz” ordene detener el trabajo. La operación `filter()` se limita a verificar si estos cinco números (del 1 al 5) son impares. Solo tres lo son, y esos son los que se imprimen, dando `135`.

> Ahora, ¿qué crees que imprime esto?
...
> La respuesta correcta es `11233455`. A medida que pasa el primer elemento, el `1` aparece en el `peek()` y se imprime. El segundo elemento pasa `limit()` y `peek()`, pero es capturado en `filter()`. El tercer y quinto elementos se comportan como el primero. El cuarto se comporta como el segundo.

> Invertir el orden de las operaciones intermedias cambia el resultado:
...
> La fuente sigue siendo un _stream_ infinito de números. El primer elemento todavía fluye por toda la canalización, y `limit()` recuerda que permite pasar un elemento. El segundo elemento no pasa el filtro. El tercer elemento fluye por toda la canalización y `limit()` permite pasar su segundo elemento. Esto continúa hasta que llega el noveno elemento y `limit()` ha permitido pasar su quinto elemento.

> Finalmente, ¿qué crees que imprime esto?
...
>  La respuesta es `1133557799`. Dado que `filter()` está antes que `peek()`, solo vemos los números impares.
