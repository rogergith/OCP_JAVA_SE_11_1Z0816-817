...
Supongamos que no queremos una `List` como valor en el mapa y preferimos un `Set` en su lugar. No hay problema. Hay otra firma de método que nos permite pasar un _downstream collector_. Este es un segundo _collector_ que hace algo especial con los valores.
...
Incluso podemos cambiar el tipo de `Map` devuelto mediante otro parámetro adicional:
...
Esto es muy flexible. ¿Qué pasa si queremos cambiar el tipo de `Map` devuelto pero dejar el tipo de los valores como `List`? No existe un método específicamente para esto porque es bastante fácil escribirlo con los métodos existentes.
...
El particionado es un caso especial de agrupación. En particionado, solo hay dos grupos posibles: `true` y `false`. El particionado es como dividir una lista en dos partes.

Supongamos que estamos haciendo un cartel para poner fuera de la exhibición de cada animal. Tenemos dos tamaños de carteles. Uno puede acomodar nombres con cinco o menos caracteres, y el otro es necesario para los nombres más largos. Podemos particionar la lista según el tipo de cartel que necesitamos.
...
Aquí pasamos un `Predicate` con la lógica para determinar a qué grupo pertenece cada nombre de animal. Ahora, supongamos que hemos descubierto cómo usar una fuente diferente, y ahora siete caracteres pueden caber en el cartel más pequeño. No hay problema. Solo cambiamos el `Predicate`.
...
Observa que todavía hay dos claves en el mapa, una para cada valor booleano. Sucede que uno de los valores es una lista vacía, pero sigue ahí. Al igual que con `groupingBy()`, podemos cambiar el tipo de `List` a otro tipo.
...
A diferencia de `groupingBy()`, no podemos cambiar el tipo de `Map` que se devuelve. Sin embargo, dado que solo hay dos claves en el mapa, ¿realmente importa qué tipo de `Map` usamos?

En lugar de usar el _downstream collector_ para especificar el tipo, podemos usar cualquiera de los _collectors_ que ya hemos mostrado. Por ejemplo, podemos agrupar por la longitud del nombre del animal para ver cuántos de cada longitud tenemos.
...
> **DEPURACIÓN DE GENERICOS COMPLICADOS**  
> Al trabajar con `collect()`, a menudo hay muchos niveles de genéricos, lo que hace que los errores del compilador sean difíciles de leer. Aquí tienes tres técnicas útiles para manejar esta situación:

> 1. Comienza con una declaración simple y sigue agregando a ella. Al hacer un pequeño cambio a la vez, sabrás qué código introdujo el error.
> 2. Extrae partes de la declaración en declaraciones separadas. Por ejemplo, intenta escribir `Collectors.groupingBy(String::length, Collectors.counting());`. Si compila, sabrás que el problema está en otra parte. Si no compila, tendrás una declaración mucho más corta para solucionar el problema.
> 3. Usa comodines genéricos para el tipo de retorno de la declaración final; por ejemplo, `Map<?, ?>`. Si ese cambio por sí solo permite que el código compile, sabrás que el problema radica en que el tipo de retorno no es el que esperabas.


Finalmente, existe un _collector_ `mapping()` que nos permite bajar un nivel y agregar otro _collector_. Supongamos que queremos obtener la primera letra del primer animal en orden alfabético de cada longitud. ¿Por qué? Tal vez para una muestra aleatoria. Los ejemplos en esta parte del examen también son bastante artificiales. Escribiríamos lo siguiente:
...
No vamos a decir que este código sea fácil de leer. Lo que sí te diremos es que es lo más complicado que necesitas entender para el examen. Al compararlo con el ejemplo anterior, puedes ver que reemplazamos `counting()` con `mapping()`. Sucede que `mapping()` toma dos parámetros: la función para el valor y cómo agruparlo aún más.

Es posible que veas los _collectors_ usados con una importación estática para hacer el código más corto. El examen incluso podría usar `var` para el valor de retorno y menos sangrías de las que usamos. Esto significa que podrías ver algo como esto:
...
El código hace lo mismo que en el ejemplo anterior. Esto significa que es importante reconocer los nombres de los _collectors_ porque puede que no tengas el nombre de la clase `Collectors` para llamar tu atención sobre ello.

> Hay un _collector_ más llamado `reducing()`. No necesitas conocerlo para el examen. Es una reducción general en caso de que todos los _collectors_ anteriores no satisfagan tus necesidades.
